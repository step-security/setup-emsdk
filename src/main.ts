...
async function run() {
  try {
    ...
    let emsdkCmd = path.join(emsdkFolder, "emsdk-main", "emsdk");
    let emsdkBaseArgs: string[] = [];

    if (os.platform() === "win32") {
      emsdkCmd = "powershell";
      emsdkBaseArgs = [path.join(emsdkFolder, "emsdk-main", "emsdk.ps1")];
    }
    ...
    if (!foundInCache) {
      if (emArgs.update) {
        await exec.exec(emsdkCmd, [...emsdkBaseArgs, "update"]);
      }

      await exec.exec(emsdkCmd, [...emsdkBaseArgs, "install", emArgs.version]);
    ...
    await exec.exec(emsdkCmd, [...emsdkBaseArgs, "activate", emArgs.version]);
    ...
    await exec.exec(emsdkCmd, [...emsdkBaseArgs, "construct_env"], {
